...
export const TextInput = ({ label=null, handleClick, buttonLabel="Send" }:any) => {
    const [value, setValue] = useState<any>()
    return (<div style={{ marginTop: 16, marginBottom: 8 }}>
        {label &&<label>{label}</label>}
        <input type="text" onChange={(e) => setValue(e.target.value)} />
        <button onClick={() => handleClick(value)} >{buttonLabel}</button>
    </div>)
}

export const SelectInput = ({ label, choices, handleClick, buttonLabel }) => {
    const [value, setValue] = useState<any>()
